...
export function AdminNavHeader({ activeTab, brandActions }: AdminNavHeaderProps) {
  return (
      ...
      <nav className="flex items-center gap-1 bg-[#F3F3F3] p-1.5 rounded-[16px] overflow-x-auto">
        {navItems.map((item) => {
          ...
          const className = isActive
            ? 'flex items-center gap-1.5 px-4 py-2 rounded-[12px] text-xs font-bold bg-[#E16349] text-white shadow-sm transition-all shrink-0'
            : 'flex items-center gap-1.5 px-4 py-2 rounded-[12px] text-xs font-semibold text-zinc-500 hover:text-[#1A1A1A] transition-all shrink-0';
