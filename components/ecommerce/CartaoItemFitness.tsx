...
export function CartaoItemFitness({ produto }: CardProps) {
  ...
  return (
    <div className="bg-white border border-zinc-200 rounded-[20px] shadow-sm overflow-hidden transition-all">
      ...
      >
        <div className="w-[88px] h-[88px] rounded-2xl flex items-center justify-center flex-shrink-0 bg-green-50 text-green-500 shadow-inner overflow-hidden">
          {produto.imagem_url ? (
            <Image
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-zinc-600 text-xs mt-1 line-clamp-2 leading-relaxed font-thin">
            {produto.descricao || 'Prato balanceado, preparado com ingredientes frescos e selecionados.'}
          </p>
          <span className="font-thin text-[#16A34A] text-[24px] block mt-2 leading-none">
            {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(Number(produto.preco_venda))}
          </span>
        </div>

        <div className="shrink-0 p-1 text-[#16A34A]">
          <svg
                ...
                return (
                  <label
                    ...
                    className={`w-full bg-white border rounded-xl p-3 flex justify-between items-center shadow-2xs transition-all ${
                      selecionado ? 'border-[#16A34A]' : 'border-zinc-200 hover:border-zinc-400'
                    }`}
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      <input
                        ...
                        className="h-5 w-5 accent-[#16A34A] rounded border-zinc-300"
                      />
          ...
          <div className="pt-2 flex justify-between items-center border-t border-zinc-100/80">
            ...
            {qtd > 0 ? (
              ...
              <button
                type="button"
                onClick={() => adicionarItem(produto, complementosSelecionados)}
                className="bg-[#DCFCE7] hover:bg-[#BBF7D0] text-[#166534] font-thin text-sm px-6 py-2.5 rounded-xl transition-all tracking-wide border border-[#BBF7D0]"
              >
                Adicionar à sacola
              </button>
            )}
